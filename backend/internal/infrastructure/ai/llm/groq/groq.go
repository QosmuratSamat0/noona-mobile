...
func NewProvider(apiKey, url, model string) (*Provider, error) {
	apiKey = strings.TrimSpace(apiKey)
	if apiKey == "" {
		return nil, fmt.Errorf("groq llm: empty api key")
	}
	url = strings.TrimSpace(url)
	if url == "" {
		return nil, fmt.Errorf("groq llm: empty url")
	}
	model = strings.TrimSpace(model)
	if model == "" {
		return nil, fmt.Errorf("groq llm: empty model")
	}
	return &Provider{
		apiKey:     apiKey,
		url:        url,
...
func (p *Provider) StreamReply(ctx context.Context, transcript, cefrLevel string) (<-chan string, error) {
	...
	go func() {
		...
		if err != nil {
			slog.Warn("groq stream reply failed", "error", err)
			return
		}
...
Focus on the single most important mistake. Keep it short.`},
	...
	if err := json.Unmarshal([]byte(sanitizeJSON(text)), &feedback); err != nil {
		return nil, fmt.Errorf("groq quick feedback malformed json: %w", err)
	}
...
func (p *Provider) Analyze(ctx context.Context, transcript string) (*linguistic.AIAnalysis, error) {
	...
	for attempt := 0; attempt < 3; attempt++ {
		if attempt > 0 {
			...
			slog.Info("retrying groq analysis", "attempt", attempt+1, "backoff", backoff)
...
Only output valid JSON.`},
	...
	if err := json.Unmarshal([]byte(sanitizeJSON(text)), &analysis); err != nil {
		return nil, fmt.Errorf("groq analysis malformed json: %w", err)
	}
...
func (p *Provider) complete(ctx context.Context, messages []message, jsonMode bool) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("groq llm: marshal request: %w", err)
	}

	req, err := http.NewRequestWithContext(ctx, http.MethodPost, p.url, bytes.NewReader(body))
	if err != nil {
		return "", fmt.Errorf("groq llm: build request: %w", err)
	}
	...
	if err != nil {
		return "", fmt.Errorf("groq llm: read response: %w", err)
	}

	var parsed chatResponse
	if err := json.Unmarshal(respBytes, &parsed); err != nil {
		return "", fmt.Errorf("groq llm: unmarshal response: %w", err)
	}
	...
	if len(parsed.Choices) == 0 {
		return "", fmt.Errorf("groq llm: empty choices")
	}
...
func mapHTTPError(statusCode int, message string) error {
	...
	case http.StatusTooManyRequests:
		return fmt.Errorf("retryable error (quota): groq llm returned %d: %s", statusCode, message)
	case http.StatusRequestTimeout, http.StatusBadGateway, http.StatusServiceUnavailable, http.StatusGatewayTimeout:
		return fmt.Errorf("retryable error (network): groq llm returned %d: %s", statusCode, message)
	default:
		return fmt.Errorf("groq llm returned %d: %s", statusCode, message)
	}
}
